...
export default function ProductOutboundFailedInspectionsPage() {
  ...
  const { inspections, statusMap, failedIds, loading } = useProductInspections(serial, dateRange);
  const list = useMemo(
    () =>
      inspections.filter(
  ...
  return (
    <InspectionsDataTable
      data={list}
      downloadCsvFileName={`product-${productId}-outbound-failed-inspections.csv`}
      downloadCsv={(rows) => ({
        headers: [
          "id",
          "created_at",
